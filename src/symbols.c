...
int inserirSimbolo(const char* nome, const char* tipo, Classe classe, Escopo escopo, int tamanho) {
    ...
    tabela[nSimbolos].estado = ESTADO_ATIVO;
...
Simbolo* buscarSimbolo(const char* nome, Escopo escopo) {
    ...
    for (int i = nSimbolos - 1; i >= 0; i--) {
        ...
        if (strcmp(tabela[i].nome, nome) == 0 && tabela[i].estado == ESTADO_ATIVO) {
...
void limparEscopo(Escopo escopo) {
    ...
    if (escopo == ESC_LOCAL) {
        for (int i = nSimbolos - 1; i >= 0; i--) {
            ...
            if (tabela[i].escopo == ESC_LOCAL && tabela[i].estado == ESTADO_ATIVO) {
...
void imprimirTabela() {
    ...
    for (int i = 0; i < nSimbolos; i++) {
        ...
        const char* estadoStr = (tabela[i].estado == ESTADO_ATIVO) ? "ATIVO" : "ZUMBI"; 
